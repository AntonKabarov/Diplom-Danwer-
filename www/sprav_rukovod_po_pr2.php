	...
	<div id="wrapper">
	   <div id="leftCol">
	        ...
	        <div id="gal">
					<div class="item">	 
<p10><?php 
$result=mysql_query("SELECT `rukovod_predpr`, `id_doljnost`, `rab_telefon` FROM `rukovod_pr_na_predpr` WHERE `id_predpr`='2' ORDER BY `rukovod_predpr`  ");
//вывод должности
$result2=mysql_query("SELECT  `rukovod_pr_na_predpr`.`rukovod_predpr` ,  `doljnost`.`doljnost` FROM  `rukovod_pr_na_predpr` 
JOIN  `doljnost` ON  `rukovod_pr_na_predpr`.`id_doljnost` =  `doljnost`.`id` WHERE `id_predpr`='2' ORDER BY `rukovod_predpr`");

$result3=mysql_query("SELECT  `rukovod_pr_na_predpr`.`rukovod_predpr` ,  `predpr`.`predpr` FROM  `rukovod_pr_na_predpr` 
JOIN  `predpr` ON  `rukovod_pr_na_predpr`.`id_predpr` =  `predpr`.`id_predpr` WHERE `predpr`='ВНИИЭФ' ORDER BY `rukovod_predpr`");
