...
ON `osnovnai`.`id_special`=`special`.`id_special` WHERE `id_gruppa`='7' ORDER BY `FIO` ");

$_result2=mysql_query("SELECT  `osnovnai`.`FIO` ,  `gruppa`.`gruppa` 
FROM  `osnovnai` 
JOIN  `gruppa` ON  `osnovnai`.`id_gruppa` =  `gruppa`.`id_gruppa` WHERE `gruppa`='ИТМ-15Д' ORDER BY `FIO` ");

$_result4=mysql_query("SELECT  `osnovnai`.`FIO` ,  `praktik`.`praktik` 
FROM  `osnovnai` 
JOIN  `praktik` ON  `osnovnai`.`id_praktik` =  `praktik`.`id_praktik` WHERE `id_gruppa`='7' ORDER BY `FIO`");

$_result5=mysql_query("SELECT  `osnovnai`.`FIO` ,  `predpr`.`predpr` 
FROM  `osnovnai` 
JOIN  `predpr` ON  `osnovnai`.`id_predpr` =  `predpr`.`id_predpr` WHERE `id_gruppa`='7' ORDER BY `FIO` ");

$_result6=mysql_query("SELECT  `osnovnai`.`FIO` ,  `podrazd`.`podrazd` 
FROM  `osnovnai` 
JOIN  `podrazd` ON  `osnovnai`.`id_podrazd` =  `podrazd`.`id_podrazd` WHERE `id_gruppa`='7' ORDER BY `FIO`");

$_result7=mysql_query("SELECT  `osnovnai`.`FIO` ,  `rukovod_pr_na_predpr`.`rukovod_predpr` 
FROM  `osnovnai` 
JOIN  `rukovod_pr_na_predpr` ON  `osnovnai`.`id_rukovod` =  `rukovod_pr_na_predpr`.`id_rukovod_predpr` WHERE `id_gruppa`='7' ORDER BY `FIO`");
