   ...
   <?php require_once "blocks/header.php" ?>
	<div id="wrapper">
	   <div id="leftCol">
	    <p>ВТ-34Д </p>
	        <div id="tehn">	 
			<?php $_result=mysql_query("SELECT `osnovnai`.`FIO`, `special`.`name_special` FROM `osnovnai` JOIN `special`
ON `osnovnai`.`id_special`=`special`.`id_special` WHERE id_gruppa=1  ");
...
ON `osnovnai`.`data_sdachi`=`data_sdachi`.`id_data_sdachi` WHERE id_gruppa=1 ");
...
"</td><td>",mysql_result($_result4,$i,data_sdachi), "</td></tr>"; echo "</table>"; 
?> 	        </div>	   </div>
